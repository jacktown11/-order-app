<template>
  <div class="goods">
    <div
      class="menu-wrapper menu-wrapper-hook"
      ref="menu"
    >
      <ul v-if="goods.length>0">
        <li
          class="category-wrapper border-bottom-1px category-wrapper-hook"
              ...
              <div class="food-info">
                ...
                <p class="price">
                  ...
                  <span
                    v-if="food.oldPrice"
                    class="old"
                  >&#00165;<span class="number">{{food.oldPrice}}</span></span>
                </p>
    ...
    <o-shopcart
      ...
      ref="cartContent"
    ></o-shopcart>
    <transition name="show-food">
      <div
        v-show="isShowFood"
        class="o-food-wrapper"
      >
        <o-food
          :food="selectedFood"
          @hide-food="hideFood"
...
import OFood from '@comp/o-food/o-food';
...
export default {
    ...
    this.$http.get('/api/goods').then(res => {
      ...
      if (data.errNum === OK_ERRNUM) {
        ...
        this.$nextTick(() => {
          ...
          this._initScroll();
          this._elInMenu = this.$refs.menu.getElementsByClassName(
            'category-wrapper-hook'
          );
          this._elInGoods = this.$refs.goods.getElementsByClassName(
            'food-category-list-hook'
          );
        });
      }
    });
  ...
  methods: {
    ...
    _calculateHeight() {
      ...
      let categoryList = this.$refs.goods.getElementsByClassName(
        'food-category-list-hook'
      );
      arr.push(height);
...
.goods
  ...
  .show-food-enter, .show-food-leave-to
    transform: translate3d(100%, 0, 0)

  .show-food-enter-active, .show-food-leave-active
    transition: all 0.3s
</style>
